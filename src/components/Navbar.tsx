...
import { companyLinks, featureLinks, links } from "../config/utils";
...
const Navbar = () => {
  ...
  return (
    <nav className="relative flex justify-between items-center px-8 py-6">
      {isOpen && (
        <div className="overlay transition max-sm:block hidden fixed z-10 top-0 left-0 w-full min-h-screen bg-neutral-900/70" />
      )}

      <button
        onClick={handleClick}
        className="hidden max-sm:block absolute top-7 right-6 z-30 text-near-black"
      >
      ...
      <div
        className={`flex z-20 items-center w-full duration-[400ms] max-sm:fixed max-sm:inset-y-0 max-sm:px-6 max-sm:right-0 max-sm:left-[30%] max-sm:overflow-auto max-sm:pb-8 max-sm:flex-col max-sm:h-full max-sm:items-start max-sm:bg-off-white max-sm:pt-[4em] transition ${
            ...
            <Dropdown title="Features" className="-left-[72px] w-full md:w-max">
              {featureLinks.map((link) => (
                ...
                >
                  ...
                  <a href="#" className="text-lg font-medium text-medium-gray">
                    {link.name}
                  </a>
                </div>
              ))}
